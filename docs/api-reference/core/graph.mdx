---
title: "graph"
description: "ZX graph construction, manipulation, and preparation for sampling."
---
...
- `tuple[BaseGraph, np.ndarray]` — A tuple containing: - The modified graph (same object, mutated in place) - A binary matrix of shape (num_f, num_e) where entry [i, j] = 1 means f_i depends on e_j. For example, if row 0 is [0, 1, 0, 1], then f0 = e1 XOR e3.
